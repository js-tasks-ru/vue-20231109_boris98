...
export default {
  ...
  render() {
    const vnodeSlots = this.$slots.default?.() ?? [];

    const tabs = vnodeSlots.filter((vnode) => vnode.type.name === 'UiTab');

    const activeTab = tabs.find((vnode) => vnode.props.name === this.active);
